...
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_event_index_unique ON event (index);

-- name: CreateIndexType :exec
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_event_index_type ON event (index ASC, type);

-- name: CreateIndexCreated :exec
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_event_created_at ON event (created_at ASC);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_event_reactor_reactor ON event_reactor (reactor);

-- name: TakeTransactionEventLock :exec
WITH i AS (SELECT max(index) idx FROM event)
SELECT pg_advisory_xact_lock_shared(COALESCE(idx, 0))
FROM i;

-- name: InsertEvent :exec
INSERT INTO event
    (id, type, data, created_at, created_by)
VALUES (pggen.arg('id'),
        pggen.arg('type'),
        pggen.arg('data'),
        pggen.arg('createdAt'),
...
WITH new_event AS (
    INSERT INTO
        ...
        WHERE e.id NOT IN (
            SELECT event_id
            FROM event_reactor r
            WHERE r.reactor = pggen.arg('reactor')::text
        )
          AND e.index >= lower(pggen.arg('range')::int8range)
          AND (upper(pggen.arg('range')) IS NULL OR e.index < upper(pggen.arg('range')))
          AND (e.type = ANY (pggen.arg('types')) OR pggen.arg('types') IS NULL)
        ORDER BY index
...
SELECT *
FROM event evt
WHERE evt.index >= lower(pggen.arg('range')::int8range)
  AND (upper(pggen.arg('range')) IS NULL OR evt.index < upper(pggen.arg('range')))
