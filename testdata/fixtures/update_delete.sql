...
UPDATE products SET price = 19.99, updated_at = now() WHERE category = 'sale';
...
UPDATE sessions SET active = false WHERE id IN (SELECT id FROM expired) RETURNING id;
...
DELETE FROM tokens WHERE expires_at < now() RETURNING id, user_id;

-- DELETE with CTE
WITH old_users AS (SELECT id FROM users WHERE last_login < now() - interval '1 year')
DELETE FROM sessions WHERE user_id IN (SELECT id FROM old_users) RETURNING session_id;
