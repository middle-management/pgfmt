...
BEGIN
	FOR source_trigger_row IN SELECT
		st.source_trigger_id,
		st.workflow_id,
		w.name AS workflow_name
	FROM
		studio.source_trigger AS st
		JOIN studio.workflow AS w ON w.workflow_id = st.workflow_id
	WHERE
		EXISTS (
			SELECT
				1
			FROM
				studio.workflow_step AS ws
			WHERE
				ws.workflow_id = st.workflow_id
		) LOOP
		new_workflow_id := studio_internal.generate_id('workflow');
		INSERT INTO
			studio.workflow (workflow_id, name)
		VALUES (new_workflow_id, source_trigger_row.workflow_name);
		UPDATE studio.source_trigger
		SET
			workflow_id = new_workflow_id
		WHERE
			source_trigger_id = source_trigger_row.source_trigger_id;
