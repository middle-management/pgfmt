...
INSERT INTO event  (id, type, data, created_at, created_by) VALUES (pggen.arg('id'), pggen.arg('type'), pggen.arg('data'), pggen.arg('createdAt'), pggen.arg('createdBy')) ;
...
FROM
	event_reactor AS r 
WHERE
	r.reactor = pggen.arg('reactor')::"text" )) AND (e.index >= lower(pggen.arg('range')::"int8range")) AND ((upper(pggen.arg('range')) IS NULL) OR (e.index < upper(pggen.arg('range')))) AND ((e.type = ANY (pggen.arg('types'))) OR (pggen.arg('types') IS NULL)) ORDER BY index   ON CONFLICT (event_id , reactor ) DO NOTHING RETURNING event_id 
)
SELECT 
	* 
FROM
	event AS evt
	JOIN new_event ON id = event_id  ORDER BY evt.index  ;
...
FROM
	event AS evt 
WHERE
	(evt.index >= lower(pggen.arg('range')::"int8range")) AND ((upper(pggen.arg('range')) IS NULL) OR (evt.index < upper(pggen.arg('range')))) AND ((evt.type = ANY (pggen.arg('types'))) OR (pggen.arg('types') IS NULL)) ORDER BY evt.index  ;
