...
SELECT * FROM a JOIN b USING (x) JOIN c ON c.id = b.id LEFT JOIN d USING (y) AS dx;

SELECT * FROM t1 NATURAL LEFT JOIN t2;

SELECT * FROM (t1 JOIN t2 ON t1.a = t2.a) AS jj;

SELECT 1 AS "my alias", CURRENT_USER AS "user" FROM "My Table" AS "T"("Col A", "Col B");

SELECT "Weird Col" FROM sch."Mixed Case";

WITH "my cte"("a col") AS (SELECT 1) SELECT * FROM "my cte";
