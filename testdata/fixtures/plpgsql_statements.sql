...
CREATE FUNCTION dynamic_rows() RETURNS SETOF int LANGUAGE plpgsql AS $$
BEGIN
  RETURN QUERY EXECUTE 'select 1' USING 2;
END
$$;

CREATE FUNCTION raise_variants() RETURNS void LANGUAGE plpgsql AS $$
BEGIN
  BEGIN
...
CREATE FUNCTION slice_walk(arr int[]) RETURNS void LANGUAGE plpgsql AS $$
DECLARE
  row_slice int[];
BEGIN
  <<slices>>
