...
-- UNION ALL
SELECT id, email FROM users WHERE active = true UNION ALL SELECT id, email FROM archived_users;

-- INTERSECT
SELECT user_id FROM orders WHERE total > 100 INTERSECT SELECT user_id FROM reviews WHERE rating >= 4;
